...
def make_answer(num_len):
    """ 3자리 서로 다른 숫자로 정답 문자열 생성 """
    answer = random.sample("0123456789", num_len)
...
def valid_guess(guess, num_len):
    ...
    - 중복 숫자가 없는가?
    """
    return guess.isdigit() and len(guess) == num_len and len(set(guess)) == num_len
...
attempts = 0 # 시도 횟수
LENGTH = 2              # 자리수 설정
answer = make_answer(LENGTH)  # 정답 생성
#print(answer) # 정답 확인용

while True :  # 언제끝날지 모르는 사용자 입력(무한반복)
    #num_len = input("게임할 숫자 자리수를 지정해주세요(LENGTH): ")
    num_len = LENGTH
    guess = input(f"{num_len}자리 숫자 입력(중복 불가) : ")
    
    # 유효성 검사
    if not valid_guess(guess, num_len):
        print(f"형식 오류 : 숫자만 중복없이 {num_len}자리로 입력하세요.")
