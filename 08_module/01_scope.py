...
#   -> "함수 블록" 안(if, for문 블록은 해당않됨; cf)java는 해당되서 java와 다른점), 특정 모듈(.py), 특정 클래스/인스턴스에서 선언된 변수 
...
def test2(a):
    glo = a + 10 # 지역변수 glo (전역변수 glo를 쓰고 있지 않다.) ==> 전역변수와 지역변수 같이 있으면 가깝게 선언된 지역변수가 우선
    b = a + glo + bal # 
...
# - immutable타입(int, float, bool, str, tuple; 불변타입) : call by value ("값"이 복사되어 처리, 자바의 깊은복사)
# - mutable (원본값 바로바로 바꿀수 있는 애들: list, dict, set) : call by reference ("참조값"이 공유되어 처리, 자바의 얕은 복사)
...
def test3(a): # a는 int(immutable) ==> call by value
    ...
    print("id(a): local a= ", id(a)) # id(a): local a=  140715781192840
    
a = 100
test3(a)
print(a) # 100
print("id(a), global a=: ", id(a)) # id(a), global a=:  140715781189640
...
def test4(b): # ==> b는 "주소값" ==> call by reference
    ...
    print("id(b): inside function b= ", id(b)) # id(b): inside function b=  2270594491456
    
b = [1,2,3]
test4(b)
print(b) # [1, 200, 3]
print("id(b): outside function b= ", id(b)) # id(b): outside function b=  2270594491456